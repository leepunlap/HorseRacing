...
-- ─── Horse eval text cache ────────────────────────────────────────────────
-- 馬評-style commentary on why a particular horse won / lost / placed in a
-- given race. Generated by betting.eval_reason which combines structured
-- race data (running positions, time, lbw, form history, odds) with an
-- optional DeepSeek call when the DEEPSEEK_API_KEY env var is set. Cached
-- here so the SPA only pays the latency / token cost once per horse-race.
CREATE TABLE IF NOT EXISTS horse_eval_text (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    race_id INTEGER NOT NULL REFERENCES races(id),
    brand TEXT NOT NULL,
    ...
    text TEXT NOT NULL,
    structured_json TEXT,                -- raw data used to build the text
    computed_at TEXT DEFAULT CURRENT_TIMESTAMP,
