...
async def global_search(q: str = "", auth = Depends(verify_token)):
    ...
    horses = [dict(r) for r in db.execute(
        "SELECT brand, name, age, sex, rating, race_count FROM horses WHERE name LIKE ? OR brand LIKE ? LIMIT 20",
...
async def list_horses(
    ...
    params = []
    if name:
        where.append("h.name LIKE ?"); params.append(f"%{name}%")
    if brand:
    ...
    allowed_sorts = {"name":"h.name","brand":"h.brand","age":"h.age","rating":"h.rating",
                     "races":"h.race_count","wins":"wins"}
    sort_col = allowed_sorts.get(sort, "h.rating")
    order_dir = "DESC" if order == "desc" else "ASC"

    sql = f"""SELECT h.brand, h.name, h.age, h.sex, h.rating, h.race_count as races,
        COALESCE(w.wins,0) as wins, COALESCE(w.win_rate,0) as win_rate
        FROM horses h
        LEFT JOIN results r ON h.brand = r.brand
